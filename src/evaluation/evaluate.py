...
def evaluate(ref_frame, pred_frame):
    ...
    try:
        ...
        LOGGER.info("Zero devision error when calculating mAUC")
        mauc = float('NaN')
...
def main():
    ...
    ref = misc.read_csv(args.reference)
    pred = misc.read_csv(args.prediction)
    print(ref)
    print(pred)
    result = evaluate(
        ref, pred)
